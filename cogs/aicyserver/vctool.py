    ...
    async def setup(self, member, after):
        if len(after.channel.members) == 1:
            if after.channel == self.bot.vc1:
                ...
                embed = discord.Embed(title="dashboard", colour=discord.Colour(0x1122a6), description="ユーザーでも変えれるようになってます")
                embed.set_footer(text='"a!vctool"でダッシュボードを再送信できます')
...
# 作り方がよくわからんから放置
    ...
    async def change(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if result == 'vc1':
            ...
            await view.wait()
            await self.bot.vc1_dash.delete()
            self.bot.vc1_dash = await self.bot.vc1.send('オーナーを変更しました', view=dashboard(self))
            self.bot.vc1_owner = member
            ...
            await view.wait()
            await self.bot.vc2_dash.delete()
            self.bot.vc2_dash = await self.bot.vc2.send('オーナーを変更しました', view=dashboard(self))
            self.bot.vc2_owner = member
            await owner.change(self, member)
        elif result == 'vc3':
            ...
            await view.wait()
            await self.bot.vc3_dash.delete()
            self.bot.vc3_dash = await self.bot.vc3.send('オーナーを変更しました', view=dashboard(self))
            self.bot.vc3_owner = member
            await owner.change(self, member)
        else:
            await interaction.response.send_message('VCのオーナーではないため実行できません', ephemeral=True)'''
