...
class Server(commands.Cog):
    ...
    async def serverinfo(self, ctx, guild:int=None):
        ...
        embed.add_field(name='サーバーブースト数', value=guild.premium_subscription_count)
        embed.add_field(name='メンバー数', value=f'{guild.member_count}')
        embed.add_field(name='サーバー作成日', value=guild.created_at.strftime("%Y/%m/%d %H:%M:%S"))
